...
import { spawn, type ChildProcessWithoutNullStreams } from "child_process" // Added
...
import { IpcService } from "../../services/ipc/IpcService"; // Added for Telegram IPC
...
export class ClineProvider extends EventEmitter<ClineProviderEvents> implements vscode.WebviewViewProvider {
  ...
  public ipcService?: IpcService; // Added for Telegram IPC
	...
	constructor(
		...
		const webSocketUrl = config.get<string>("telegram.webSocketUrl") || "ws://localhost:8765"; // Get URL from VS Code config
		this.webSocketClient = new WebSocketClient(this.outputChannel); // Pass only the logger
		// Connection is handled internally by the WebSocketClient constructor
	
		// Removed IpcService initialization
	
		// WebSocket listener is handled within Cline.ts
	}
	...
	*/
	async dispose() {
		...
		this.webSocketClient?.dispose() // Dispose WebSocketClient
		// Removed stopIpcListener call
	...
	async resolveWebviewView(webviewView: vscode.WebviewView | vscode.WebviewPanel) {
		...
		// Always use the production build in the Extension Development Host for now
		// TODO: Add a more robust way to detect if HMR server is actually running if needed
    ...
    // Removed startIpcListener call (WebSocket handles connection)
  this.outputChannel.appendLine("Webview view resolved")
	...
	public async initClineWithTask(task?: string, images?: string[], parentTask?: Cline) {
		...
		// Pass WebSocketClient to Cline constructor
		// Pass services in the options object
		const cline = new Cline({
			webSocketClient: this.webSocketClient,
			// Removed ipcService from options
			provider: this,
			apiConfiguration,
			...
			onCreated: (cline: Cline) => this.emit("clineCreated", cline), // Added type annotation
		})
	...
	public async initClineWithHistoryItem(historyItem: HistoryItem & { rootTask?: Cline; parentTask?: Cline }) {
		...
		// Pass WebSocketClient to Cline constructor when resuming from history
		// Pass services in the options object
		const cline = new Cline({
			webSocketClient: this.webSocketClient,
			// Removed ipcService from options
			...
			async (message: WebviewMessage) => {
				switch (message.type) {
					case "webviewDidLaunch":
						...
						// Removed call to startIpcListener (ensuring it's gone)
					...
					case "checkpointDiff":
						...
						if (result.success) {
							// await this.getCurrentCline()?.checkpointDiff(result.data) // Commented out - Property 'checkpointDiff' does not exist on type 'Cline'.
						}
	...
	async getStateToPostToWebview() {
		...
		return {
			currentTaskId: this.getCurrentCline()?.taskId, // Add current task ID
	...
	*/
	...
	public log(message: string) {
		...
		// Use console.log for internal provider logging
		console.log(message)
		// Use outputChannel for user-visible logs
		this.outputChannel.appendLine(message)
	}
